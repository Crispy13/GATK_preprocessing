...
# MarkDuplicates
MD_input=$(find -maxdepth 1 -name "*_merged.bam" | sed -r "s/(\.\/)/--INPUT \1/g")
...
gatk ApplyBQSR -R $r -I ${MD_output} -bqsr ${MD_output/.bam/recal_data.table} -O ${MD_output/.bam/recal_reads.bam}

echo -e "\n> Makes post_recal_data.table for ${MD_output} file.\n"
gatk BaseRecalibrator -R $r -I ${MD_output/.bam/recal_reads.bam} --known-sites /data/eck/Data/GATK/bundle/hg19/1000G_phase1.indels.hg19.sites.vcf --known-sites /data/eck/Data/GATK/bundle/hg19/Mills_and_1000G_gold_standard.indels.hg19.sites.vcf --known-sites /data/eck/Data/GATK/bundle/hg19/dbsnp_138.hg19.vcf -O ${MD_output/.bam/post_recal_data.table}

echo -e "\n> Starts to generate before/after plots for ${MD_output} file.\n"
gatk AnalyzeCovariates -before ${MD_output/.bam/recal_data.table} -after ${MD_output/.bam/post_recal_data.table} -plots ${MD_output/.bam/_AnalyzeCovariates.pdf}
